...
function tblsave.serialize(value, pretty)
	local kw = {
		["and"] = true,
		["break"] = true,
		["do"] = true,
...
function tblsave.unserialize(data)
...
function tblsave.load(path)
	checkArg(1, path, "string")
	
	local content, err = fs.readFile(path)
	if not content then return nil, err end

	return tblsave.unserialize(content)
end

function tblsave.save(path, data)
	checkArg(1, path, "string")
	checkArg(2, data, "table")
	return fs.writeFile(path, tblsave.serialize(data))
end
