...
static void rawSandbox(lua_State* lua, canvas_t* canvas) {
	...
	LUA_PUSH_BOOL(ENV_KEYBOARD_ENABLED);
	LUA_PUSH_INT(SOUND_BEEPCARD_CHANNELS);
	LUA_PUSH_INT(SOUND_NOISECARD_CHANNELS);
	LUA_PUSH_INT(SOUND_SOUNDCARD_CHANNELS);
	...
	LUA_BIND_RETR(hal_filesystem_sdcardGetSize, (), LUA_RET_INT);
...
static void _lua_hook(lua_State *L, lua_Debug *ar) {
